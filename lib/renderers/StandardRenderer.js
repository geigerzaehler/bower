...
StandardRenderer.prototype.end = function (data) {
    var method = '_' + camelCase(this._command);
...
StandardRenderer.prototype.log = function (log) {
    var method = '_' + camelCase(log.id) + 'Log';
...
StandardRenderer.prototype._install = function (packages) {
    ...
    forOwn(packages, function (pkg) {
        var cliTree;

        // List only 1 level deep dependencies
        forOwn(pkg.dependencies, function (dependency) {
...
StandardRenderer.prototype._prefix = function (log) {
    ...
    if (this._compact) {
        ...
        return idColor(rpad(id, this._sizes.id));
    }
    ...
    return chalk.green(label) + repeat(' ', nrSpaces) + idColor(id);
};
...
StandardRenderer.prototype._tree2archy = function (node) {
    var dependencies = values(node.dependencies);
    ...
    return {
        label: label,
        nodes: values(dependencies).map(this._tree2archy, this)
    };
};
