...
var orderByDependencies = function (packages, installed, json) {
    ...
    var depsSatisfied = function (packageName) {
        return difference(keys(packages[packageName].dependencies), installed, ordered).length === 0;
    };

    var depsFromBowerJson = json && json.dependencies ? keys(json.dependencies) : [];
    var packageNames = keys(packages);

    //get the list of the packages that are specified in bower.json in that order
    //its nice to maintain that order for users
    var desiredOrder = intersection(depsFromBowerJson, packageNames);
    //then add to the end any remaining packages that werent in bower.json
    desiredOrder = desiredOrder.concat(difference(packageNames, desiredOrder));
    ...
    while (resolvedOne) {
        ...
        for (var i = 0; i < desiredOrder.length; i++) {
            ...
            if (depsSatisfied(packageName)) {
                ...
                remove(desiredOrder, packageName);
...
var run = function (cmdString, action, logger, config) {
    ...
    var env = mixIn({ 'BOWER_PID': process.pid }, process.env);
    var args = shellquote.parse(cmdString, env);
    var cmdName = args[0];
    remove(args, cmdName); //no rest() in mout
...
var hook = function (action, ordered, config, logger, packages, installed, json) {
    if (keys(packages).length === 0 || !config.scripts || !config.scripts[action]) {
        /*jshint newcap: false  */
        return Q();
    }

    var orderedPackages = ordered ? orderByDependencies(packages, installed, json) : keys(packages);
    var cmdString = replace(config.scripts[action], '%', orderedPackages.join(' '));
    return run(cmdString, action, logger, config);
};

module.exports = {
    preuninstall: partial(hook, 'preuninstall', false),
    preinstall: partial(hook, 'preinstall', true),
    postinstall: partial(hook, 'postinstall', true),
