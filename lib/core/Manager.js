...
var find = require('mout/object/find');
...
Manager.prototype.configure = function (setup) {
    ...
    this._targets.forEach(function (decEndpoint) {
        ...
        decEndpoint.dependants = values(decEndpoint.dependants);
    ...
    this._resolved = {};
    this._installed = {};
    forOwn(setup.resolved, function (decEndpoint, name) {
        decEndpoint.dependants = values(decEndpoint.dependants);
    ...
    // Installed
    mixIn(this._installed, setup.installed);
...
Manager.prototype.install = function (json) {
    ...
    .then(function () {
        ...
        forOwn(that._dissected, function (decEndpoint, name) {
            var promise;
            var dst;
    ...
    .then(function () {
        ...
        forOwn(that._dissected, function (pkg) {
            // Sync dependencies
            forOwn(pkg.dependencies, function (dependency, name) {
        ...
        return map(that._dissected, function (decEndpoint) {
...
Manager.prototype.toData = function (decEndpoint, extraKeys, upperDeps) {
    ...
    upperDeps = upperDeps || [];
    data.endpoint = pick(decEndpoint, ['name', 'source', 'target']);
    ...
    if (extraKeys) {
        extra = pick(decEndpoint, extraKeys);
        extra = filter(extra, function (value) {
            return !!value;
        });
        mixIn(data, extra);
    }
    ...
    data.nrDependants = size(decEndpoint.dependants);
...
Manager.prototype._onFetchSuccess = function (decEndpoint, canonicalDir, pkgMeta, isTargetable) {
    ...
    index = findIndex(resolved, function (resolved) {
...
Manager.prototype._onFetchError = function (decEndpoint, err) {
    ...
    err.data = err.data || {};
    err.data.endpoint = pick(decEndpoint, ['name', 'source', 'target']);

    // Remove from being fetched list
    remove(this._fetching[name], decEndpoint);
    this._nrFetching--;
...
Manager.prototype._parseDependencies = function (decEndpoint, pkgMeta, jsonKey) {
    ...
    forOwn(pkgMeta[jsonKey], function (value, key) {
        var resolved;
        var fetching;
        var compatible;
        ...
        if (resolved) {
            ...
            compatible = find(resolved, function (resolved) {
        ...
        if (fetching) {
            compatible = find(fetching, function (fetching) {
...
Manager.prototype._dissect = function () {
    ...
    if (this._hasFailed) {
        ...
        err = values(this._failed)[0][0];
        this._deferred.reject(err);
        return;
    }

    // Find a suitable version for each package name
    forOwn(this._resolved, function (decEndpoints, name) {
        var semvers;
        var nonSemvers;
    ...
    .then(function () {
        ...
        forOwn(this._resolutions, function (resolution, name) {
        ...
        this._dissected = filter(suitables, function (decEndpoint, name) {
...
Manager.prototype._electSuitable = function (name, semvers, nonSemvers) {
    ...
    // compatible with every requirement
        ...
        suitable = find(semvers, function (subject) {
    ...
    unresolvable = find(picks, function (pick) {
    ...
    if (resolution && !unresolvable) {
        ...
        if (semver.validRange(resolution)) {
            suitable = findIndex(picks, function (pick) {
    ...
    return Q.nfcall(this._logger.prompt.bind(this._logger), {
        ...
        validate: function (choice) {
            choice = Number(trim(choice.trim(), '!'));
