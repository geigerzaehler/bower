        ...
        if (that._options.save || that._options.saveDev) {
            ...
            decEndpoints.forEach(function (decEndpoint) {
                ...
                if (that._options.save) {
                    that._json.dependencies = mixIn(that._json.dependencies || {}, jsonEndpoint);
                }

                if (that._options.saveDev) {
                    that._json.devDependencies = mixIn(that._json.devDependencies || {}, jsonEndpoint);
                }
            });
        }
        ...
        names.forEach(function (name) {
            ...
            .then(function () {
                ...
                that.walkTree(tree, function (node, nodeName) {
                    if (name === nodeName) {
                        dependants.push.apply(dependants, values(node.dependants));
                    }
                }, true);

                // Remove duplicates
                dependants = unique(dependants);
                ...
                dependantsNames = unique(dependantsNames);
...
Project.prototype.getTree = function () {
    ...
    .spread(function (json, tree, flattened) {
        ...
        // Convert flattened
        flattened = map(flattened, function (node) {
...
Project.prototype.walkTree = function (node, fn, onlyOnce) {
    var result;
    var dependencies;
    var queue = values(node.dependencies);
...
Project.prototype._analyse = function () {
    ...
    .spread(function (json, installed, links) {
        var root;
        var jsonCopy = deepClone(json);
        var flattened = mixIn({}, installed, links);
        ...
        forOwn(installed, function (decEndpoint, key) {
        ...
        forOwn(flattened, function (decEndpoint, name) {
...
Project.prototype._bootstrap = function (targets, resolved, incompatibles) {
    var installed = map(this._installed, function (decEndpoint) {
    ...
    .then(function () {
        ...
        if (!size(this._json.resolutions)) {
...
Project.prototype._removePackages = function (packages) {
    ...
    .then(function () {
        ...
        forOwn(packages, function (dir, name) {
            var promise;
    ...
    .then(function () {
        return filter(packages, function (dir) {
            return !!dir;
        });
    });
};
...
Project.prototype._restoreNode = function (node, flattened, jsonKey) {
    ...
    deps = filter(node.pkgMeta[jsonKey], function (value, key) {
        return !node.dependencies[key];
    });

    forOwn(deps, function (value, key) {
        ...
        // This might change in the future: #673
            ...
            if (node.root && compatible) {
                originalSource = get(local, 'pkgMeta._originalSource');
